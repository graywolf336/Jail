...
public class PlayerListener implements Listener {
    ...
    public void jailStickHandling(EntityDamageByEntityEvent event) {
        if(pl.getConfig().getBoolean(Settings.JAILSTICKENABLED.getPath())) {
            ...
            if(!pl.getJailStickManager().isValidStick(attacker.getInventory().getItemInHand().getType())) return;
            
            //They don't have permission for that type of jail stick
            if(!attacker.hasPermission("jail.usejailstick." + attacker.getInventory().getItemInHand().getType().toString().toLowerCase())) return;
